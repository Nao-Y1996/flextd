...
def filter_dicts_by_inclusion(origins: list[dict], key: str, value_list: list[str]) -> list[dict]:
    """
...
def filter_dicts_by_exclusion(origins: list[dict], key: str, value_list: list[str]) -> list[dict]:
    """
...
def filter_dataset(annotation_file: str,
                   include_files: list[str] = None,
                   exclude_files: list[str] = None,
                   include_categories: list[str] = None,
                   exclude_categories: list[str] = None) -> dict | None:
    """
    ...
    if (include_files is None) and (exclude_files is None) and (
            include_categories is None) and (exclude_categories is None):
        return None
    ...
    if include_files is not None or exclude_files is not None:
        ...
        if include_files is not None:
            new_images = filter_dicts_by_inclusion(new_images, key="file_name", value_list=include_files)
        if exclude_files is not None:
            new_images = filter_dicts_by_exclusion(new_images, key="file_name", value_list=exclude_files)
        # fix annotation list
        new_image_ids = [image["id"] for image in new_images]
        new_annotations = filter_dicts_by_inclusion(annotation_dict["annotations"], key="image_id",
    ...
    if include_categories is not None or exclude_categories is not None:
        ...
        if include_categories is not None:
            new_categories = filter_dicts_by_inclusion(new_categories, key="name", value_list=include_categories)
        if exclude_categories is not None:
            new_categories = filter_dicts_by_exclusion(new_categories, key="name", value_list=exclude_categories)

        # fix annotation list
        new_category_ids = [category["id"] for category in new_categories]
        new_annotations = filter_dicts_by_inclusion(annotation_dict["annotations"], key="category_id",
                                                    value_list=new_category_ids)

        # fix image list
        new_image_ids = [annotation["image_id"] for annotation in new_annotations]
        new_images = filter_dicts_by_inclusion(annotation_dict["images"], key="id", value_list=new_image_ids)
...
def create_filtered_annotation_file(annotation_file: str,
                                    ...
                                    exclude_files: list[str] = None,
                                    include_categories: list[str] = None,
                                    exclude_categories: list[str] = None) -> str:
    """
    ...
    annotation_dict = filter_dataset(annotation_file=annotation_file,
